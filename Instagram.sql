...
-- Q - How many times does the average user post?
	...
	select Users.id, count(Photos.user_id) as No_of_Posts 
...
-- Q - Find the percentage of our users who have either never commented on a photo or have commented on every photo.
	...
	select avg(Not_Null_Likes) as Avg_Not_Null_Likes from (select count(id) as Not_Null_Likes from No_of_Likes where No_of_Likes != 0 ) as Not_Null_Likes;
    
	select  as Avg_Most_Likes
    from (select count(id) as Most_Likes from No_of_Likes
    group by id having max(No_of_Likes)) as Most_Likes;
...
Never_Commented AS (
    SELECT COUNT(DISTINCT Users.id) AS Users_Never_Commented
    FROM Users
    LEFT JOIN Comments ON Users.id = Comments.user_id
    WHERE Comments.user_id IS NULL
),
Commented_Every_Photo AS (
    SELECT COUNT(user_id) AS Users_Commented_Every_Photo
    FROM (
        SELECT user_id
        FROM Comments
        GROUP BY user_id
        HAVING COUNT(DISTINCT photo_id) = (SELECT COUNT(*) FROM Photos)
    ) AS Users_Commented_All_Photos
)
SELECT 
    ((Never_Commented.Users_Never_Commented + Commented_Every_Photo.Users_Commented_Every_Photo) * 100.0 / Total.Total_Users) AS Percentage
FROM Total, Never_Commented, Commented_Every_Photo;